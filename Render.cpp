...
Window::Window() {
	if (SDL_Init(SDL_INIT_EVERYTHING) > 0) {
		...
		window = SDL_CreateWindow("Super Shoot", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_SHOWN);
...
void Window::render(SDL_Texture* Layer, int x, int y, int Width, int Height, SDL_Rect* clip) {
	...
	SDL_RenderCopy(renderer, Layer, clip, &renderQuad);
}
void Window::renderEx(SDL_Texture* Layer, int x, int y, int Width, int Height, SDL_Rect* clip, double angle) {
	// Set rendering space and render to screen
	SDL_Rect renderQuad = { x, y, Width, Height };
	SDL_RenderCopyEx(renderer, Layer, clip, &renderQuad, angle, NULL, SDL_FLIP_NONE);
}
